...
exports.caster = function(req, res){
  if(req.params.type="audio"){
    if(audioCasterCode.length){
      for(var i=0;i<audioCasterCode.length;i++){
        if(audioCasterCode[i]=req.params.code){res.render('caster', { title: 'Audio Caster', name:audioCasterName[i]}); return true;}
      }
  ...
  if(videoCasterCode.length){
      for(var i=0;i<videoCasterCode.length;i++){
        if(videoCasterCode[i]=req.params.code){res.render('caster', { title: 'Video Caster', name:videoCasterName[i]}); return true;}
      }
